  ...
  return {
    ...
    integrations: {
      Core: {
        ...
        CreateFileSignedUrl: async ({ file_path }) => {
          console.warn("CreateFileSignedUrl called with:", file_path);
          const mockUrl = `https://mock-storage.supabase.co/signed/${Date.now()}_${file_path}`;
          return { url: mockUrl };
        },

        UploadPrivateFile: async ({ file, path }) => {
          console.warn("UploadPrivateFile called with:", file?.name, path);
          const mockUrl = `https://mock-storage.supabase.co/private/${Date.now()}_${file?.name || "file"}`;
