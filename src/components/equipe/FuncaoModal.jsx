...
export default function FuncaoModal({ isOpen, onClose, onSave, funcao, setores, atividadesSalvas }) {
  ...
  useEffect(() => {
    if (isOpen) {
      if (isEditing && funcao) {
        setFormData({
          nome: funcao.nome || '',
          setores_ids: Array.isArray(funcao.setores_ids) ? funcao.setores_ids : [],
          objetivo_central: funcao.objetivo_central || '',
          objetivo_geral: funcao.objetivo_geral || '',
          atribuicoes: Array.isArray(funcao.atribuicoes) ? funcao.atribuicoes : [], // Garantir que seja array
          atividades_salvas_ids: Array.isArray(funcao.atividades_salvas_ids) ? funcao.atividades_salvas_ids : [],
          links: Array.isArray(funcao.links) ? funcao.links : [],
          imagens: Array.isArray(funcao.imagens) ? funcao.imagens : []
        });
      } else {
  ...
  const handleSubmit = (e) => {
    ...
    onSave(formData, isEditing ? funcao.id : null);
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <Label>Atividades Salvas</Label>
            <MultiSelectDropdown options={atividadesSalvas.map(a => ({ value: a.id, label: a.titulo }))} selectedValues={formData.atividades_salvas_ids} onChange={(v) => handleInputChange('atividades_salvas_ids', v)} placeholder="Selecione atividades"/>
          </div>
                  ...
                  {formData.imagens.length < 5 && (
                    <div className="text-center">
                      <Button 
                        ...
                        onClick={() => document.getElementById('funcao-image-upload').click()}
                      >
